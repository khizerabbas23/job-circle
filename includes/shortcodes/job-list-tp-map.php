<?php
function jobcircle_job_list_frten() {
    vc_map(
       array(
             'name' => __( 'Job list 14' ),
             'base' => 'jobcircle_job_list_frten',
...
add_action( 'vc_before_init', 'jobcircle_job_list_frten' );
...
function jobcircle_job_list_frten_frontend( $atts, $content ) {
...
<div class="visual-map <?php echo esc_html($mapfilter) ?>">
    		...
    		if($atts['map_embed_selection'] == 'top_map_with_filter'){
    								...
    								<div class="form-input">
										...
										if (!empty($cat_terms)) {
											foreach ($cat_terms as $cat_term) {
												?>
    										<option><?php echo esc_html($cat_term->name) ?></option>
    									<?php 
											}
										}
										?>
    			...
    			<?php } 
    			elseif($atts['map_embed_selection'] == 'top_map_with_search'){?>
    			<div class="visual-map-filters">
				<div class="container">
					<!-- search form -->
					<form class="form-search form-inline" action="#">
						<div class="fields-holder bg-white text-black d-flex flex-wrap flex-md-nowrap">
							<div class="form-group">
								...
								<input class="form-control" type="search" placeholder="<?php esc_html_e('Search Job Title','jobcircle-frame')?>">
							</div>
							<div class="form-group">
								<i class="jobcircle-icon-map-pin icon"></i>
								<select class="select2 select2-hidden-accessible" name="state" data-placeholder="Select Country" data-select2-id="select2-data-1-1qsl" tabindex="-1" aria-hidden="true">
									<option label="Placeholder" data-select2-id="select2-data-3-lk8a"></option>
									<option value="AF"><?php esc_html_e('Afghanistan','jobcircle-frame')?></option>									
								</select><span class="select2 select2-container select2-container--default" dir="ltr" data-select2-id="select2-data-2-0dlg" style="width: 139.391px;"><span class="selection"><span class="select2-selection select2-selection--single" role="combobox" aria-haspopup="true" aria-expanded="false" tabindex="0" aria-disabled="false" aria-labelledby="select2-state-uv-container" aria-controls="select2-state-uv-container"><span class="select2-selection__rendered" id="select2-state-uv-container" role="textbox" aria-readonly="true" title="Select Country"><span class="select2-selection__placeholder">Select Country</span></span><span class="select2-selection__arrow" role="presentation"><b role="presentation"></b></span></span></span><span class="dropdown-wrapper" aria-hidden="true"></span></span>
							</div>
						</div>
						<button class="btn btn-primary" type="submit"><span class="btn-text"><?php esc_html_e('Search Now','jobcircle-frame')?></span></button>
					</form>
				</div>
			</div>
					...
					<aside class="filters-sidebar custom-filters">
					    <button class="btn btn-filters filters-opener opener-active" type="button">
							...
							<div class="filter-buttons">
								...
								<input type="hidden" name="action" value="jobcircle_jobs_listing_forten_ajax"><button type="submit" class="btn btn-green btn-sm"><span class="btn-text"><?php esc_html_e('Apply Filter','jobcircle-frame')?></span></button>
								<a href="#" class="btn btn-text btn-sm"><?php esc_html_e('Reset all filters','jobcircle-frame')?></a>
							</div>
						</form>
					</aside>
					<div class="row jobcircle-alljobs-list">
					    <?php	echo jobcircle_jobs_listing_forten($atts); ?></div>
					</div>
					</div>
			</section>
			<?php
...
add_shortcode( 'jobcircle_job_list_frten', 'jobcircle_job_list_frten_frontend' );

function jobcircle_jobs_listing_forten($atts){
	ob_start();
	
	global $job_map_items;	global $job_map_items;
        ...
        if ( $query->have_posts() ) {
			...
			<div class="col-12 col-md-6 mb-15 mb-md-30">
                <article class="featured-box">
					<a href="" class="pin-job <?php echo esc_ttml($like_btn_class)  ?> " data-id="<?php echo esc_ttml($postid) ?>"><i class=" <?php echo esc_ttml($fav_icon_class) ?> position-absolute <?php echo esc_ttml($follow) ?>"></i></a>
					...
					<?php if(!empty($permalinkget) && !empty($title)){ ?>
						...
						if(!empty($categories)){
							...
							if($count_category == 0 ){ ?>
							  <strong class="subtitle"><span class="txt"><?php echo esc_html($category->name); ?></span></strong>
						<?php 	
							}else{
									        break;
									    }
        ...
        <div class="col-12 col-md-6 col-xl-4 mb-15 mb-md-30">
									...
									<article class="featured-box">
										...
										<?php if(!empty($permalinkget) && !empty($title)){ ?>
											...
											<span class="icon"><img src="<?php echo esc_url_raw($job_img_url) ?>" width="43" height="43" alt="Creative Hut"></span>
											<?php }	
									if(!empty($categories)){
									    $count_category = 0;
							foreach ($categories as $category){  
						...
						<?php 
									   ...
									   $count_category++; 
											...
											<a href="javascript:;" class="jobcircle-det-applybtn jobdet-apply btn-act btn btn-primary jobcircle-aplybtn-con" data-id="<?php echo esc_html($job_id) ?>"
data-bs-toggle="modal" data-bs-target="#jobcircle-apply-job-popup"><span class="btn-text"><?php echo esc_html('Apply Now', 'jobcircle-frame') ?></span></a>
										</div>
									</article>
								</div>
                <?php
...
data: {
	post_id: post_id,
	action: 'jobcircle_candidate_fav_jobcircle_job_list_frten'
},
success: function(data) {
...
data: {
post_id: post_id,
action: 'jobcircle_candidate_remove_jobcircle_job_list_frten'
},
success: function(data) {
var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_jobs_listing_forten_ajax', 'jobcircle_jobs_listing_forten_ajax');
add_action('wp_ajax_nopriv_jobcircle_jobs_listing_forten_ajax', 'jobcircle_jobs_listing_forten_ajax');

function jobcircle_jobs_listing_forten_ajax(){
	...
	$html = jobcircle_jobs_listing_forten($atts);
...
add_action('wp_ajax_jobcircle_candidate_fav_jobcircle_job_list_frten', 'jobcircle_candidate_fav_jobcircle_job_list_frten');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_jobcircle_job_list_frten', 'jobcircle_candidate_fav_jobcircle_job_list_frten');
function jobcircle_candidate_fav_jobcircle_job_list_frten() {
    global $current_user;
    $user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_jobcircle_job_list_frten', 'jobcircle_candidate_remove_jobcircle_job_list_frten');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_jobcircle_job_list_frten', 'jobcircle_candidate_remove_jobcircle_job_list_frten');
function jobcircle_candidate_remove_jobcircle_job_list_frten() {
    global $current_user;
    $user_id = $current_user->ID;
    $post_id = $_POST['post_id'];
