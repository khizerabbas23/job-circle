<?php
function jobcircle_job_discover_job()
{
      $terms = get_terms(
		array(
	...
	vc_map(
		array(
			'name' => __('Discover Job'),
			'base' => 'jc_job_discover_job',
...
add_action('vc_before_init', 'jobcircle_job_discover_job');
// popular category frontend
function jobcircle_job_discover_job_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
				...
				<div class="row <?php echo esc_html($sidebaralign); ?>">
					...
					<?php  }  ?>		
						...
						<div class="row justify-content-center  jobcircle-alljobs-list">
							<?php
							echo jobcircle_jobs_simple_discover_job($atts);  ?>
						</div></div>
					</div>
				</div>
			</div>
...
add_shortcode('jc_job_discover_job', 'jobcircle_job_discover_job_frontend');
function jobcircle_jobs_simple_discover_job($atts)
{
	ob_start();
	$jobcircle_page = isset($atts['jobcircle_page']) ? $atts['jobcircle_page'] : '';
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			...
			if (isset($_REQUEST['view']) && $_REQUEST['view'] == 'grid') {  
			    if($countr == 0){ ?>
			<section class="section section-theme-16 theme_sevenb">
				<div class="jobs_info_wrap row">
                 <?php } ?>
				<div class="col-12 col-md-6 col-xl-6 mb-15 mb-xl-40">
								<div class="jobs_info_holder discover-post">
...
if($countr == 0){ ?>
						<section class="section section-theme-17 theme_sevenb">
					<div class="jobs_info_wrap">
					    <?php } ?>
				<div class="col-12 col-lg-12 mb-15 mb-xl-30 d-md-flex">
	<div class="jobs_info_holder">
									<span class="badge">
										<a href="" class="pin-job <?php echo esc_html($like_btn_class); ?> " data-id="<?php echo esc_html($post); ?>"><i class=" <?php echo esc_html($fav_icon_class); ?> position-relative <?php echo esc_html($follow); ?>"></i></a>	
									</span>
...
add_action('wp_ajax_jobcircle_jobs_simple_discover_job_one', 'jobcircle_jobs_simple_discover_job_one');
add_action('wp_ajax_nopriv_jobcircle_jobs_simple_discover_job_one', 'jobcircle_jobs_simple_discover_job_one');
function jobcircle_jobs_simple_discover_job_one()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simple_discover_job($atts);
