<?php
function jobcircle_job_freelance_job()
{
	vc_map(
		array(
			'name' => __('Freelance Job'),
			'base' => 'jc_job_freelance_job',
...
add_action('vc_before_init', 'jobcircle_job_freelance_job');
// popular category frontend
function jobcircle_job_freelance_job_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
		...
		$atts,
		'jobcircle_job_freelance_job'
	);
	$orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	$atts['listin_ajax_action'] = 'jobcircle_jobs_simple_freelance_job_one';
	wp_enqueue_script('jobcircle-jobfunctions');
	$output = '';
	ob_start();
	?>
	<section class="section section-categories section-theme-1 pt-35 pt-md-50 pt-lg-75 pt-xl-95 pb-0 pb-xxl-35">
		<?php
		$pageview = '';
				...
				<div class="row <?php echo esc_html($sidebaralign); ?>">
					...
					<?php  }  ?>		
						...
						<div class="row justify-content-center  jobcircle-alljobs-list">
							...
							<?php 
							echo jobcircle_jobs_simple_freelance_job($atts);  ?>
						</div></div>
					</div>
				</div>
			</div>
...
add_shortcode('jc_job_freelance_job', 'jobcircle_job_freelance_job_frontend');
function jobcircle_jobs_simple_freelance_job($atts)
{
	ob_start();
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	$orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			...
			if (isset($_REQUEST['view']) && $_REQUEST['view'] == 'grid') {
      if($countr == 0) {	?>
	 <section class="section section-theme-2 featured-categories pt-0 pt-md-0 pt-xxl-65 pb-0 pb-xxl-0">
				...
				<div class="col-12 col-md-6 mb-15 mb-md-30">
							...
							<article class="featured-category-box">
                                ...
                                <a href="<?php echo jobcircle_esc_the_html($permalinkget)?>"><strong class="h6"><?php echo esc_html($title);?></strong></a>
                                    <span class="subtitle"><?php esc_html_e('By','jobcircle-frame')?> <a href="<?php echo  $post_author_link ?>" class= "textdecore"> <?php echo esc_html($post_author_name);?> </a></span>
									...
									<div class="job-info">
										...
										<span class="amount"><strong><?php echo esc_html($job_salary);?></strong>
                                        </span>
									</div>
...
if($countr == 0) {	?>
	  <section class="section section-theme-12 featured_Jobs_Block freelancepost pt-0 pb-0">
...
add_action('wp_ajax_jobcircle_jobs_simple_freelance_job_one', 'jobcircle_jobs_simple_freelance_job_one');
add_action('wp_ajax_nopriv_jobcircle_jobs_simple_freelance_job_one', 'jobcircle_jobs_simple_freelance_job_one');
function jobcircle_jobs_simple_freelance_job_one()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simple_freelance_job($atts);
