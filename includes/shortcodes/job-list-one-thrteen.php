<?php
function jobcircle_listed_job_thirteen()
{
	vc_map(
		array(
			'name' => __('Listed Job 13'),
			'base' => 'jobcircle_listed_job_thirteen',
...
add_action('vc_before_init', 'jobcircle_listed_job_thirteen');
...
function jobcircle_listed_job_thirteen_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
						...
						<div class="col-12 col-lg-7 pt-35 pt-md-50 pt-lg-75 pb-35 pb-md-50 pb-xl-75">
							...
							<aside class="filters-sidebar custom-filters">
								...
								<form method="post" class="jobcircle-jobfilter-form">
									...
									<div class="filter-box">
										...
										<div class="form-group search-field">
											<input class="form-control" name="keyword"
												placeholder="<?php echo esc_html_e('Search with keyword', 'jobcircle-frame') ?>"
												type="text">
									...
									<div class="filter-box">
										<h2 class="h5">
											<?php echo esc_html_e('Location', 'jobcircle-frame') ?>
										</h2>
										<div class="form-group search-field">
											<input class="form-control jobcircle-location-input-field" name="location"
												placeholder="<?php echo esc_html_e('Enter any location', 'jobcircle-frame') ?>"
												type="text">
										</div>
									</div>
									<div class="filter-box">
										<a class="filter-box-head" data-bs-toggle="collapse" href="#collapseCategory">
											<h2 class="h5">
												<?php echo esc_html_e('Category', 'jobcircle-frame') ?>
											</h2>
									...
									<div class="filter-buttons">
										...
										<input type="hidden" name="action"
											value="jobcircle_simple_job_filters_new_submit_call_thirteen"><button
											type="submit" class="btn btn-green btn-sm"><span class="btn-text">
												<?php esc_html_e('Apply Filter', 'jobcircle-frame') ?>
											</span></button>
							...
							<div class="row justify-content-center jobcircle-alljobs-list">
								<?php echo jobcircle_jobs_simple_new_list_thirteen($atts); ?>
							</div>
						</div>
...
add_shortcode('jobcircle_listed_job_thirteen', 'jobcircle_listed_job_thirteen_frontend');
function jobcircle_jobs_simple_new_list_thirteen($atts)
{
	ob_start();
					...
					<div class="job-card alt">
						...
						<div class="card-footer">
							<?php if (!empty($job_img_url)) { ?>
								<div class="img"><img src="<?php echo esc_url_raw($job_img_url) ?>" width="68" height="68" alt="image">
								</div>
							<?php } ?>
			...
			<?php } else { ?>
				<div class="col-12 col-md-6 col-lg-12 col-xl-6 mb-15 mb-md-30">
					...
					<article class="featured-box">
						<a href="" class="pin-job <?php echo esc_html($like_btn_class); ?> " data-id="<?php echo esc_html($post); ?>"><i
						...
						<div class="textbox">
							...
							<strong class="subtitle">
								<?php if (!empty($job_img_url)) { ?>
									<span class="icon"><img src="<?php echo esc_url_raw($job_img_url) ?>" width="43" height="43"
											alt="Creative Hut"></span>
								<?php }
								if (!empty($categories)) {
								    $count_category = 0;
									foreach ($categories as $category) { 
			...
			<script>
				jQuery(document).on('click', '.jobcircle-follower-btn', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_fav_jobcircle_listed_job_thirteen'
						},
						success: function (data) {
				...
				jQuery(document).on('click', '.jobcircle-alrdy-favjab', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_remove_job_jobcircle_listed_job_thirteen'
						},
						success: function (data) {
							var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_simple_job_filters_new_submit_call_thirteen', 'jobcircle_simple_job_filters_new_submit_call_thirteen');
add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_new_submit_call_thirteen', 'jobcircle_simple_job_filters_new_submit_call_thirteen');
function jobcircle_simple_job_filters_new_submit_call_thirteen()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simple_new_list_thirteen($atts);
	wp_send_json(array('html' => $html));
}
// for favourite button
add_action('wp_ajax_jobcircle_candidate_fav_jobcircle_listed_job_thirteen', 'jobcircle_candidate_fav_jobcircle_listed_job_thirteen');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_jobcircle_listed_job_thirteen', 'jobcircle_candidate_fav_jobcircle_listed_job_thirteen');
function jobcircle_candidate_fav_jobcircle_listed_job_thirteen()
{
	global $current_user;
	$user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_job_jobcircle_listed_job_thirteen', 'jobcircle_candidate_remove_job_jobcircle_listed_job_thirteen');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_job_jobcircle_listed_job_thirteen', 'jobcircle_candidate_remove_job_jobcircle_listed_job_thirteen');
function jobcircle_candidate_remove_job_jobcircle_listed_job_thirteen()
{
	global $current_user;
	$user_id = $current_user->ID;
	$post_id = $_POST['post_id'];
