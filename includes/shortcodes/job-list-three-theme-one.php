<?php
function jobcircle_job_list_listing() {
    vc_map(
       array(
             'name' => __( 'Job List 40' ),
             'base' => 'jobcircle_job_list_listing',
...
add_action( 'vc_before_init', 'jobcircle_job_list_listing' );
...
function jobcircle_job_list_listing_frontend( $atts, $content ) {
    ...
    ), $atts, 'jobcircle_job_list_listing'
    );
			...
			<section class="section section-categories pt-35 pt-md-50 pt-lg-75 pt-xl-95 pb-35 pb-md-50 pb-xl-75">
				...
				<div class="container<?php echo esc_html($pageview) ?>">
					...
					<header class="page-subheader mb-30 mb-md-40 d-lg-flex align-items-center justify-content-between">
						...
						if(!empty($numofpost) && !empty($numofpost)){  	
								?>
						<h1 class="h4 mb-25 mb-lg-0"><?php echo esc_html_e('Showing 1 to', 'jobcircle-frame') ?> <?php echo esc_html($numofpost) ?> <?php echo esc_html_e('of', 'jobcircle-frame') ?> <?php echo esc_html($job_count) ?> <?php echo esc_html_e('jobs', 'jobcircle-frame') ?></h1>
						<?php }  ?>
					...
					<aside class="filters-sidebar custom-filters">
							...
							<div class="filter-box">
								<a class="filter-box-head" data-bs-toggle="collapse" href="#collapseCategory">
									<h2 class="h5"><?php echo esc_html_e('Category', 'jobcircle-frame') ?></h2>
				...
				<div class="filter-box">
							...
							<div class="filter-buttons">
								...
								<input type="hidden" name="action" value="jobcircle_list_two_listing_ajax"><button type="submit" class="btn btn-green btn-sm"><span class="btn-text"><?php esc_html_e('Apply Filter','jobcircle-frame')?></span></button>
								<a href="#" class="btn btn-text btn-sm"><?php esc_html_e('Reset all filters','jobcircle-frame')?></a>
							</div>
						</form>
					</aside>
					<div class="row jobcircle-alljobs-list">				    
					<?php echo jobcircle_list_two_listing($atts); ?>
						</div>
					</div>
				</div>
			</section>
...
add_shortcode( 'jobcircle_job_list_listing', 'jobcircle_job_list_listing_frontend' );

function jobcircle_list_two_listing($atts){
	ob_start();
...
if (in_array($post, $fav_jobs)) {
	...
	$follow = 'fa fa-heart';
		          ...
		          ?>    
					<div class="col-12 col-sm-6 col-lg-4 mb-15 mb-md-30">
						...
						<article class="featured-category-box alt">
							<a href="" class="fav-tag <?php echo esc_html($like_btn_class) ?> " data-id="<?php echo esc_html($postid) ?>"><i class=" <?php echo esc_html($fav_icon_class) ?> position-absolute <?php echo esc_html($follow) ?>"></i></a>
							<?php if(!empty($permalinkget) && !empty($job_img_url)){ ?>
								...
								<?php if(!empty($permalinkget) && !empty($title)){ ?>
									<strong class="h6"><a href="<?php echo esc_html($permalinkget); ?>"><?php echo esc_html($title) ?></a></strong>
									<?php } 
									if(!empty($post_author_url) && !empty($post_author_name)){ ?>
									<span class="subtitle"><?php esc_html_e('By', 'jobcircle-frame') ?> 
									<a href="<?php echo esc_html($post_author_url); ?>"><?php echo esc_html($post_author_name) ?></a></span>
									<?php }  
									if(!empty($job_salary)){ ?>
									<div class="job-info">
				...
				<div class="jobs_info_holder">
					...
					<span class="note"><?php echo esc_html($job_type_str['title']);?></span>
					<?php } ?>
					<div class="wrap_holder">
					<?php if(!empty($permalinkget) && !empty($job_img_url)){  ?>	
						<div class="icon_holder">
							<a href="<?php echo esc_html($permalinkget); ?>">
							<img src="<?php echo esc_url_raw($job_img_url)?>" alt="job-circle">
							</a>
						</div>
						<?php }  ?>
...
data: {
	post_id: post_id,
	action: 'jobcircle_candidate_fav_job_jobcircle_job_list_listing'
},
success: function(data) {
...
data: {
post_id: post_id,
action: 'jobcircle_candidate_remove_jobjobcircle_job_list_listing'
},
success: function(data) {
var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_list_two_listing_ajax', 'jobcircle_list_two_listing_ajax');
add_action('wp_ajax_nopriv_jobcircle_list_two_listing_ajax', 'jobcircle_list_two_listing_ajax');

function jobcircle_list_two_listing_ajax(){
	...
	$html = jobcircle_list_two_listing($atts);
...
add_action('wp_ajax_jobcircle_candidate_fav_job_jobcircle_job_list_listing', 'jobcircle_candidate_fav_job_jobcircle_job_list_listing');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_jobcircle_job_list_listing', 'jobcircle_candidate_fav_job_jobcircle_job_list_listing');
function jobcircle_candidate_fav_job_jobcircle_job_list_listing() {
    global $current_user;
    $user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_jobjobcircle_job_list_listing', 'jobcircle_candidate_remove_jobjobcircle_job_list_listing');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_jobjobcircle_job_list_listing', 'jobcircle_candidate_remove_jobjobcircle_job_list_listing');
function jobcircle_candidate_remove_jobjobcircle_job_list_listing() {
    global $current_user;
    $user_id = $current_user->ID;
    $post_id = $_POST['post_id'];
