<?php
function jobcircle_job_list_ten() {  
      $terms = get_terms(
		array(
...
add_action( 'vc_before_init', 'jobcircle_job_list_ten' );
...
function jobcircle_job_list_ten_frontend( $atts, $content ) {
    $atts = shortcode_atts(
    array(
        'orderby' => '',
		'numofpost' => '',
    ...
    $atts, 'jobcircle_job_list_ten'
    );
    
 $orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
$jobcircle_page = isset($atts['jobcircle_page']) ? $atts['jobcircle_page'] : '';
$atts['listin_ajax_action'] = 'jobcircle_jobs_listing_ten_all';
...
$output ='';
 ...
 <?php 
	...
	<section class="section section-categories section-theme-1 pt-35 pt-md-50 pt-lg-75 pt-xl-95 pb-0  pb-xxl-15">
	                <?php 
					    $pageview = '';
				...
				<?php 	echo jobcircle_jobs_listing_ten($atts);  ?>						
						</div>
					</div>
				</div>
					</div>
...
add_shortcode( 'jobcircle_job_list_ten', 'jobcircle_job_list_ten_frontend' ); 

function jobcircle_jobs_listing_ten($atts){
	ob_start();
	 ...
	 while ( $query->have_posts() ) {
	  ...
	  ?>	
                       <div class="col-12 col-sm-6 col-xxl-4 mb-15 mb-md-30">
                                   ...
                                   <span class="tag"><a class="text-dark" href="<?php echo esc_url($jobcircle_page_url);?>?job_category=<?php echo jobcircle_esc_the_html($term->slug) ?>"><?php echo esc_html($term->name); ?></a></span> 
            	...
            	<div class="col-12 mb-15 mb-md-30">
					...
					<?php if(!empty($permalinkget) && !empty($job_img_url)){ ?>		
							...
							if(!empty($post_author_url) && !empty($post_author_name)){ ?>	
							<span class="meta">
								<?php esc_html_e('By', 'jobcircle-frame') ?> <a href="<?php echo esc_url_raw($post_author_url) ?>" class="myclass">
						...
						<div class="job-content-right">
							...
							<a href="<?php echo jobcircle_esc_the_html($permalinkget) ?>" class="btn btn-green btn-sm" style="background:transparent !important;"><span><?php esc_html_e('Apply Now', 'jobcircle-frame') ?></span></a>
						</div>
					</div>
				</div>
                <?php
...
add_action('wp_ajax_jobcircle_jobs_listing_ten_all', 'jobcircle_jobs_listing_ten_all');
add_action('wp_ajax_nopriv_jobcircle_jobs_listing_ten_all', 'jobcircle_jobs_listing_ten_all');

function jobcircle_jobs_listing_ten_all()
{
	...
	$html = jobcircle_jobs_listing_ten($atts);
