<?php
function jobcircle_new_grid_three_theme_job()
{
	vc_map(
		array(
			'name' => __('Grid Job 95'),
			'base' => 'jobcircle_new_grid_three_theme_job',
...
add_action('vc_before_init', 'jobcircle_new_grid_three_theme_job');
...
function jobcircle_new_grid_three_theme_job_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
			...
			'map_embed_selection' => '', ),
		$atts,
		'jobcircle_new_grid_three_theme_job'
	);
					...
					<div class="row <?php echo esc_html($mapalign); ?>">
						<div class="col-12 col-lg-7 pt-35 pt-md-50 pt-lg-75 pb-35 pb-md-50 pb-xl-75">
								...
								<form method="post" class="jobcircle-jobfilter-form">
									...
									<div class="filter-buttons">
										...
										<input type="hidden" name="action"
											value="jobcircle_simple_job_filters_new_submit_call_one_job_grid_threemap_theme_one"><button
							...
							<div class="row justify-content-center jobcircle-alljobs-list">
								<?php echo jobcircle_jobs_simplemap_left_new_grid_one($atts); ?>
							</div>
						</div>
...
add_shortcode('jobcircle_new_grid_three_theme_job', 'jobcircle_new_grid_three_theme_job_frontend');
function jobcircle_jobs_simplemap_left_new_grid_one($atts)
{
	ob_start();
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			...
			if (!empty($latitue) && !empty($longitude)) {
				...
				{
				lat:
				<?php echo jobcircle_esc_the_html($latitue); ?>,
				lng:
				<?php echo jobcircle_esc_the_html($longitude); ?>,
				imageSrc: '
				<?php echo jobcircle_esc_the_html($job_img_url); ?>',
				title: "
						...
						<div class="box-holder">
										...
										<?php if (!empty($post_author_url) && !empty($post_author_name)) { ?>
											<li><span class="text">
													<?php echo esc_html_e('By', 'jobcircle-frame') ?> <a
														href="<?php echo esc_html($post_author_url); ?>" class="myclass">
							...
							<footer class="jobs-foot">
								...
								<a href="javascript:;"
									...
									data-id="<?php echo esc_html($job_id) ?>" data-bs-toggle="modal"
			...
			<?php } else { ?>
				<div class="col-12 col-sm-6 col-xxxl-4 mb-15 mb-md-30">
					...
					<article class="featured-category-box">
						...
						if (!empty($permalinkget) && !empty($job_img_url)) { ?>
							<div class="img-holder">
								<a href="<?php echo esc_html($permalinkget); ?>">
									<img src="<?php echo esc_url_raw($job_img_url) ?>" width="78" height="78" alt="Product Manager">
								</a>
							</div>
						<?php } ?>
						<div class="textbox">
							...
							<a href="javascript:;"
								class="jobcircle-det-applybtn jobdet-applybtn-act btn btn-dark-yellow btn-sm jobcircle-aplybtn-con"
								data-id="<?php echo esc_html($post); ?>" data-bs-toggle="modal"
								data-bs-target="#jobcircle-apply-job-popup">
								<span class="btn-text"><span class="text">
										<?php echo esc_html_e('Apply Now', 'jobcircle-frame') ?>
									</span><i class="jobcircle-icon-chevron-right"></i></span></a>
						</div>
					</article>
				</div>
			<?php
			...
			<script>
				jQuery(document).on('click', '.jobcircle-follower-btn', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_fav_job_grid_nintefive'
						},
						success: function (data) {
				...
				jQuery(document).on('click', '.jobcircle-alrdy-favjab', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_remove_job_grid_nintyfive'
						},
						success: function (data) {
							var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_simple_job_filters_new_submit_call_one_job_grid_threemap_theme_one', 'jobcircle_simple_job_filters_new_submit_call_one_job_grid_threemap_theme_one');
add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_new_submit_call_one_job_grid_threemap_theme_one', 'jobcircle_simple_job_filters_new_submit_call_one_job_grid_threemap_theme_one');
function jobcircle_simple_job_filters_new_submit_call_one_job_grid_threemap_theme_one()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simplemap_left_new_grid_one($atts);
	wp_send_json(array('html' => $html));
}
// for favourite button
add_action('wp_ajax_jobcircle_candidate_fav_job_grid_nintefive', 'jobcircle_candidate_fav_job_grid_nintefive');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_grid_nintefive', 'jobcircle_candidate_fav_job_grid_nintefive');
function jobcircle_candidate_fav_job_grid_nintefive()
{
	global $current_user;
	$user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_job_grid_nintyfive', 'jobcircle_candidate_remove_job_grid_nintyfive');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_job_grid_nintyfive', 'jobcircle_candidate_remove_job_grid_nintyfive');
function jobcircle_candidate_remove_job_grid_nintyfive()
{
	global $current_user;
	$user_id = $current_user->ID;
	$post_id = $_POST['post_id'];
