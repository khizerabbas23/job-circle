<?php
function jobcircle_job_grid_five_search()
{
	vc_map(
		array(
			'name' => __('Job Grid five search'),
			'base' => 'jc_job_grid_five_search',
...
add_action('vc_before_init', 'jobcircle_job_grid_five_search');
...
function jobcircle_job_grid_five_search_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
		...
		$atts, 'jobcircle_job_grid_five_search'
	);
	$orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	$atts['listin_ajax_action'] = 'jobcircle_jobs_simple_grid_five_search_all';
	...
	ob_start();
	?>
	<div class="visual-map subvisual-theme-1">
		<div id="map"></div>
		<!-- search form -->
				...
				<form class="form-search form-inline" action="#">
					<div class="fields-holder bg-white text-black d-flex flex-wrap flex-md-nowrap">
						<div class="form-group">
							<label for="rel01">
								<?php echo esc_html_e('What are you looking for?', 'jobcircle-frame') ?>
							</label>
							<div class="form-input">
								<select id="rel01" class="select2" name="state"
									data-placeholder="<?php echo esc_html_e('What are you looking for?', 'jobcircle-frame') ?>">
						...
						<div class="form-group">
							...
							<div class="form-input">
								<select id="rel02" class="select2" name="job_category"
									data-placeholder="<?php echo esc_html_e('Category', 'jobcircle-frame') ?>">
									<option label="Placeholder"></option>
									<?php
				...
				<div class="row <?php echo esc_html($sidebaralign); ?>">
					...
					<div class="col-12 col-lg-8 col-xxl-9">
								...
								<div class="subhead-filters-item">
									...
									<form id="jobForm" class="form-group d-lg-flex align-items-center" method="get">
										<select class="select2" name="sortby"
											data-placeholder="<?php esc_html_e('Sort By', 'jobcircle-frame') ?>"
											onchange="submitForm()">
						...
						<div class="row justify-content-center  jobcircle-alljobs-list">
							<?php echo jobcircle_jobs_simple_grid_five_search($atts); ?>
						</div>
					</div>
				</div>
			</div>
...
add_shortcode('jc_job_grid_five_search', 'jobcircle_job_grid_five_search_frontend');
function jobcircle_jobs_simple_grid_five_search($atts)
{
	ob_start();
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	$orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			...
			if (!empty($latitue) && !empty($longitude)) {
				...
				imageSrc: '
				<?php echo jobcircle_esc_the_html($image[0]); ?>',
				title: "
			...
			<script>
				jQuery(document).on('click', '.jobcircle-follower-btn', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_fav_jobjobcircle_job_grid_five_search'
						},
						success: function (data) {
				...
				jQuery(document).on('click', '.jobcircle-alrdy-favjab', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_remove_jobcircle_job_grid_five_search'
						},
						success: function (data) {
							var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_jobs_simple_grid_five_search_all', 'jobcircle_jobs_simple_grid_five_search_all');
add_action('wp_ajax_nopriv_jobcircle_jobs_simple_grid_five_search_all', 'jobcircle_jobs_simple_grid_five_search_all');
function jobcircle_jobs_simple_grid_five_search_all()
{
	...
	$html = jobcircle_jobs_simple_grid_five_search($atts);
	wp_send_json(array('html' => $html));
}
// for favourite button
add_action('wp_ajax_jobcircle_candidate_fav_jobjobcircle_job_grid_five_search', 'jobcircle_candidate_fav_jobjobcircle_job_grid_five_search');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_jobjobcircle_job_grid_five_search', 'jobcircle_candidate_fav_jobjobcircle_job_grid_five_search');
function jobcircle_candidate_fav_jobjobcircle_job_grid_five_search()
{
	global $current_user;
	$user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_jobcircle_job_grid_five_search', 'jobcircle_candidate_remove_jobcircle_job_grid_five_search');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_jobcircle_job_grid_five_search', 'jobcircle_candidate_remove_jobcircle_job_grid_five_search');
function jobcircle_candidate_remove_jobcircle_job_grid_five_search()
{
	global $current_user;
	$user_id = $current_user->ID;
	$post_id = $_POST['post_id'];
