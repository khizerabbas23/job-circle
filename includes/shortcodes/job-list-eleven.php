<?php
function jobcircle_listed_job_eleven(){
    vc_map(
        array(
            'name' => __('Listed Job 11'),
            'base' => 'jobcircle_listed_job_eleven',
...
add_action('vc_before_init', 'jobcircle_listed_job_eleven');
...
function jobcircle_listed_job_eleven_frontend($atts, $content){
	$atts = shortcode_atts(
		array(
			'orderby' => '',
			'numofpost' => '',
...
<main class="main">
			...
			<section class="section section-categories section-theme-1 pb-35 pb-lg-0 pt-0">
				<div class="container<?php echo esc_html($pageview) ?>">
					...
					$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
					?>
							<header class="page-subheader mb-30 mb-md-40 d-xxl-flex align-items-center justify-content-between">
								<?php if(!empty($job_count)){ ?>
								<h3 class="h6 mb-25 mb-xxl-0"><?php esc_html_e('All','jobcircle-frame')?> <?php echo esc_html($job_count) ?> <?php esc_html_e('jobs found','jobcircle-frame')?></h3>
								<?php }  ?>
								<div class="subhead-filters d-xxl-flex align-items-center justify-content-between">
									...
									<?php 
									if(!empty($listactive)){    ?>
										<a href="?view=grid"><button class="btn <?php echo esc_html($listactive) ?>" type="button">											
										<img src="<?php echo Jobcircle_Plugin::root_url()?>/images/grid-icon.svg" width="20" height="20" alt="Grid">
										</button></a>
										<?php  }  
										if(!empty($gridactive)){   ?>
										<a href="?view=list"><button class="btn <?php echo esc_html($gridactive) ?>" type="button">										
										<img src="<?php echo Jobcircle_Plugin::root_url()?>/images/list-icon.svg" width="22" height="22" alt="List">
										</button></a>
										<?php 	}  ?>
													...
													<ul class="checkbox-list">
														...
														<li>
															<label class="custom-checkbox">
																...
																name="job_category" value="<?php echo esc_html($cat_term->slug) ?>" 
																type="checkbox">
						...
						<div class="filter-box">
							...
							<div class="filter-buttons">
								...
								<input type="hidden" name="action" value="jobcircle_simple_job_filters_new_submit_call_eleven"><a href="#" class="btn btn-green btn-sm"><span class="btn-text"><?php esc_html_e('Apply Filter','jobcircle-frame')?></span></a>
		...
		<?php	echo jobcircle_jobs_simple_new_list_eleven($atts);	?>
				</div>
			</div>
		</section>
	</main>
...
add_shortcode('jobcircle_listed_job_eleven', 'jobcircle_listed_job_eleven_frontend');

function jobcircle_jobs_simple_new_list_eleven($atts){
	ob_start();
        ...
        while ($query->have_posts()) {
            ...
            <div class="col-12 col-md-6 col-xxxl-4 mb-15 mb-md-20">
				...
				<div class="job-card alt">
					...
					echo esc_html($job_type_str['title']);
					} ?>
						</span>
					...
					<?php if(!empty($job_img_url)){	?>
						<div class="img"><img src="<?php echo esc_url_raw($job_img_url) ?>" width="68" height="68" alt="image"></div>
						<?php }  ?>
...
<a href="javascript:;" class="jobcircle-det-applybtn jobdet-applybtn-act btn btn-primary jobcircle-aplybtn-con" data-id="<?php echo esc_html($job_id) ?>"
...
data: {
	post_id: post_id,
	action: 'jobcircle_candidate_fav_jobjobcircle_listed_job_eleven'
},
success: function(data) {
...
data: {
post_id: post_id,
action: 'jobcircle_candidate_remove_jojobcircle_listed_job_eleven'
},
success: function(data) {
var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_simple_job_filters_new_submit_call_eleven', 'jobcircle_simple_job_filters_new_submit_call_eleven');
add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_new_submit_call_eleven', 'jobcircle_simple_job_filters_new_submit_call_eleven');

function jobcircle_simple_job_filters_new_submit_call_eleven()
{
	...
	$html = jobcircle_jobs_simple_new_list_eleven($atts);
...
add_action('wp_ajax_jobcircle_candidate_fav_jobjobcircle_listed_job_eleven', 'jobcircle_candidate_fav_jobjobcircle_listed_job_eleven');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_jobjobcircle_listed_job_eleven', 'jobcircle_candidate_fav_jobjobcircle_listed_job_eleven');
function jobcircle_candidate_fav_jobjobcircle_listed_job_eleven() {
    global $current_user;
    $user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_jojobcircle_listed_job_eleven', 'jobcircle_candidate_remove_jojobcircle_listed_job_eleven');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_jojobcircle_listed_job_eleven', 'jobcircle_candidate_remove_jojobcircle_listed_job_eleven');
function jobcircle_candidate_remove_jojobcircle_listed_job_eleven() {
    global $current_user;
    $user_id = $current_user->ID;
    $post_id = $_POST['post_id'];
