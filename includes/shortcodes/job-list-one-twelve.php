<?php
function job_circle_listed_twlv()
{
	vc_map(
		array(
			'name' => __('Job List 12'),
			'base' => 'job_circle_listed_twlv',
			...
			'params' => array(
				...
				array(
					...
					'value' => array(
						...
						'Top Map With Filter Style 2' => 'top_map_with_filter_style2',
					),
				),
			)
		)
...
add_action('vc_before_init', 'job_circle_listed_twlv');
...
function job_circle_listed_twlv_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
	...
	if ($atts['map_embed_selection'] !== '') {
		...
		?>	
					...
					if ($atts['map_embed_selection'] == 'top_map_with_filter') {
						...
						<?php } elseif ($atts['map_embed_selection'] == 'top_map_with_filter_style2') { ?>				
							<div class="visual-map-filters">
							<div class="container">
								<!-- search form -->
								<form class="form-search form-inline" action="#">
									<div class="fields-holder bg-white text-black d-flex flex-wrap flex-md-nowrap">
										<div class="form-group">
											...
											<input class="form-control" type="keyword" placeholder="<?php echo esc_html_e('Search Job Title', 'jobcircle-frame') ?>">
										</div>
										<div class="form-group">
											<i class="jobcircle-icon-map-pin icon"></i>
											<select class="select2" name="job_category" data-placeholder="<?php esc_html_e('Category', 'jobcircle-frame') ?>">
												<option label="Placeholder"></option>
												<?php
												$cat_terms = get_terms(
													array(
					...
					<aside class="filters-sidebar bg-light-gray">
						...
						<div class="filter-box">
								...
								<div class="filter-box">
									...
									<!-- Filter Box -->
									<div class="filter-box">
										<h2 class="h5"><?php esc_html_e('Salary Range', 'jobcircle-frame') ?></h2>
									...
									<div class="filter-buttons">
										...
										<input type="hidden" name="action" value="jobcircle_simple_job_filters_submit_call_twelv"><button type="submit" class="btn btn-primary btn-sm"><span class="btn-text submit-filters-button"><?php echo esc_html_e('Filter', 'jobcircle-frame') ?></span></button>
						...
						<?php echo jobcircle_jobs_simple_list_twlev($atts); ?>
...
add_shortcode('job_circle_listed_twlv', 'job_circle_listed_twlv_frontend');
function jobcircle_jobs_simple_list_twlev($atts)
{
	ob_start();
		...
		while ($query->have_posts()) {
			...
			if (isset($_REQUEST['view']) && $_REQUEST['view'] == 'grid') {
				?>
						...
						<?php } else { ?>
								<div class="col-12 col-md-6 col-lg-12 col-xl-6 mb-15 mb-md-30">
													<!-- Featured Box -->
									   ...
									   $count_category++; 
														...
														?>
															...
															<?php } ?>
															<a href="javascript:;" class="jobcircle-det-applybtn jobdet-applybtn-act btn btn-primary jobcircle-aplybtn-con" data-id="<?php echo esc_html($post) ?>"
				data-bs-toggle="modal" data-bs-target="#jobcircle-apply-job-popup"><span class="btn-text"><span class="text"><?php echo esc_html('Apply Now', 'jobcircle-frame') ?></span><i class="jobcircle-icon-chevron-right icon"></i></span></a>
														</div>
													</article>
												</div>
							<?php
	...
	?>
		...
		add_action('wp_footer', function () {
			...
			data: {
				post_id: post_id,
				action: 'jobcircle_candidate_fav_job_circle_listed_twlv'
			},
			success: function(data) {
			...
			data: {
			post_id: post_id,
			action: 'jobcircle_candidate_remove_job_circle_listed_twlv'
			},
			success: function(data) {
			var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_simple_job_filters_submit_call_twelv', 'jobcircle_simple_job_filters_submit_call_twelv');
add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_submit_call_twelv', 'jobcircle_simple_job_filters_submit_call_twelv');
function jobcircle_simple_job_filters_submit_call_twelv()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simple_list_twlev($atts);
	wp_send_json(array('html' => $html));
}
// for favourite button
add_action('wp_ajax_jobcircle_candidate_fav_job_circle_listed_twlv', 'jobcircle_candidate_fav_job_circle_listed_twlv');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_circle_listed_twlv', 'jobcircle_candidate_fav_job_circle_listed_twlv');
function jobcircle_candidate_fav_job_circle_listed_twlv()
{
	global $current_user;
	$user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_job_circle_listed_twlv', 'jobcircle_candidate_remove_job_circle_listed_twlv');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_job_circle_listed_twlv', 'jobcircle_candidate_remove_job_circle_listed_twlv');
function jobcircle_candidate_remove_job_circle_listed_twlv()
{
	global $current_user;
	$user_id = $current_user->ID;
	$post_id = $_POST['post_id'];
