<?php
function jobcircle_new_list_two_map()
{
	vc_map(
		array(
			'name' => __('Job List Map'),
			'base' => 'jobcircle_new_list_two_map',
...
add_action('vc_before_init', 'jobcircle_new_list_two_map');
...
function jobcircle_new_list_two_map_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
						...
						<div class="col-12 col-lg-7 pt-35 pt-md-50 pt-lg-75 pb-35 pb-md-50 pb-xl-75">
							...
							<aside class="filters-sidebar custom-filters">
								...
								<form method="post" class="jobcircle-jobfilter-form">
									...
									<div class="filter-box">
										...
										<div class="form-group search-field">
											<input class="form-control" name="keyword" placeholder="<?php esc_html_e('Search with keyword', 'jobcircle-frame') ?>"
												type="text">
									...
									<div class="filter-box">
										...
										<div class="form-group search-field">
											<input class="form-control jobcircle-location-input-field" name="location"
												placeholder="<?php esc_html_e('Enter any location', 'jobcircle-frame') ?>" type="text">
										</div>
									</div>
									<div class="filter-box">
										...
										<div class="filter-buttons">
											...
											<input type="hidden" name="action"
												value="jobcircle_jobs_simple_list_two_map_filter"><button type="submit"
								...
								<div class="row justify-content-center jobcircle-alljobs-list">
									<?php echo jobcircle_jobs_simple_list_two_map_new_filter($atts); ?>
								</div>
							</div>
...
add_shortcode('jobcircle_new_list_two_map', 'jobcircle_new_list_two_map_frontend');
function jobcircle_jobs_simple_list_two_map_new_filter($atts)
{
	ob_start();
				...
				<div class="col-12 mb-15 mb-md-30">
					...
					<div class="jobs-card">
						<div class="job-content-left">
							<?php if (($permalinkget) && !empty($job_img_url)) { ?>
								<div class="icon-box">
							...
							if (!empty($post_author_url) && !empty($post_author_name)) { ?>
								<span class="meta">
									<?php echo esc_html_e('By', 'jobcircle-frame') ?> <a href="<?php echo esc_html($post_author_url) ?>">
						...
						<div class="job-content-right">
							<div class="price-box">
								...
								if (!empty($experince)) { ?>
									<span class="txt">
										<?php echo esc_html_e('Experience', 'jobcircle-frame') ?>
										<?php echo esc_html($experince) ?>
									</span>
								<?php } ?>
							</div>
							<a href="javascript:;" class="jobcircle-det-applybtn jobdet-applybtn-act jobcircle-aplybtn-con"
								data-id="<?php echo esc_html($post) ?>" data-bs-toggle="modal"
								data-bs-target="#jobcircle-apply-job-popup"><button class="btn btn-green"><span>
...
add_action('wp_ajax_jobcircle_jobs_simple_list_two_map_filter', 'jobcircle_jobs_simple_list_two_map_filter');
add_action('wp_ajax_nopriv_jobcircle_jobs_simple_list_two_map_filter', 'jobcircle_jobs_simple_list_two_map_filter');
function jobcircle_jobs_simple_list_two_map()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simple_list_two_map_new_filter($atts);
