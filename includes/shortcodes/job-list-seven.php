<?php
function jobcircle_new_listed_job(){
    vc_map(
        array(
            'name' => __('Listed Job 7'),
            'base' => 'jobcircle_new_listed_job',
...
add_action('vc_before_init', 'jobcircle_new_listed_job');
...
function jobcircle_new_listed_job_frontend($atts, $content){
	$atts = shortcode_atts(
		array(
			'orderby' => '',
			'numofpost' => '',
					...
					?>
						...
						<div class="filter-box">
							...
							<div class="filter-buttons">
										...
										<input type="hidden" name="action" value="jobcircle_simple_job_filters_new_submit_call_one"><a href="#" class="btn btn-green btn-sm"><span class="btn-text"><?php esc_html_e('Apply Filter','jobcircle-frame')?></span></a>
							...
							<?php
						echo jobcircle_jobs_simple_new_list_one($atts); ?>
						</div>
						</div>
...
add_shortcode('jobcircle_new_listed_job', 'jobcircle_new_listed_job_frontend');

function jobcircle_jobs_simple_new_list_one($atts){
	ob_start();
        ...
        while ($query->have_posts()) {
            ...
            <div class="col-12 col-md-6 col-xxxl-4 mb-15 mb-md-20">
									...
									<div class="job-card alt">
										<div class="inner-box">
											<?php if(!empty($job_type_str) || !empty($permalinkget)) { ?>
											<span class="job-type">												
											...
											<a href="<?php echo esc_html($permalinkget) ?>"><h3><?php echo esc_html($title); ?></h3></a>
											<?php  } 
											if(!empty($job_salary)) { ?>
				...
				<article class="popular-jobs-box">
					<div class="box-holder">
						...
						<footer class="jobs-foot">
							...
							<a href="javascript:;" class="jobcircle-det-applybtn jobdet-applybtn-act btn btn-green jobcircle-aplybtn-con btn-sm" data-id="<?php echo esc_html($job_id) ?>"
...
data: {
	post_id: post_id,
	action: 'jobcircle_candidate_fav_job_listed_job_seven'
},
success: function(data) {
...
data: {
post_id: post_id,
action: 'jobcircle_candidate_remove_job_list_job_list_seven'
},
success: function(data) {
var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_simple_job_filters_new_submit_call_one', 'jobcircle_simple_job_filters_new_submit_call_one');
add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_new_submit_call_one', 'jobcircle_simple_job_filters_new_submit_call_one');

function jobcircle_simple_job_filters_new_submit_call_one()
{
	...
	$html = jobcircle_jobs_simple_new_list_one($atts);
...
add_action('wp_ajax_jobcircle_candidate_fav_job_listed_job_seven', 'jobcircle_candidate_fav_job_listed_job_seven');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_listed_job_seven', 'jobcircle_candidate_fav_job_listed_job_seven');
function jobcircle_candidate_fav_job_listed_job_seven() {
    global $current_user;
    $user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_job_list_job_list_seven', 'jobcircle_candidate_remove_job_list_job_list_seven');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_job_list_job_list_seven', 'jobcircle_candidate_remove_job_list_job_list_seven');
function jobcircle_candidate_remove_job_list_job_list_seven() {
    global $current_user;
    $user_id = $current_user->ID;
    $post_id = $_POST['post_id'];
