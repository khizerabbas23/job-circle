<?php
function jobcircle_job_open_job()
{
...
add_action('vc_before_init', 'jobcircle_job_open_job');
add_action('wp_head', function () {

});
// popular category frontend
function jobcircle_job_open_job_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
		...
		$atts,
		'jobcircle_job_open_job'
	);
	$orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	$jobcircle_page = isset($atts['jobcircle_page']) ? $atts['jobcircle_page'] : '';
	$atts['listin_ajax_action'] = 'jobcircle_jobs_simple_open_job_one';
				...
				<div class="row <?php echo esc_html($sidebaralign); ?>">
					...
					<?php  }  ?>		
						...
						<div class="row justify-content-center  jobcircle-alljobs-list">
							...
							<?php
							echo jobcircle_jobs_simple_open_job($atts);  ?>
						</div></div>
					</div>
					</div>
				</div>
...
add_shortcode('jc_job_open_job', 'jobcircle_job_open_job_frontend');
function jobcircle_jobs_simple_open_job($atts)
{
	ob_start();
    $jobcircle_page = isset($atts['jobcircle_page']) ? $atts['jobcircle_page'] : '';
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			...
			if (isset($_REQUEST['view']) && $_REQUEST['view'] == 'list') {
	       if($countr == 0){       ?>
		<section class="section section-theme-14 ">
   ...
   if($countr == 0){       ?>
			<section class="section section-theme-11 bg-white  pt-md-10 pb-35 pb-md-50 pb-lg-65 pb-xxl-100 ">
						<div class="tab-content">
                    <div class="jobs-frame">
			<?php } ?>
				<div class="jobs-card open-job-card openjobs mb-40 col-md-6">
...
add_action('wp_ajax_jobcircle_jobs_simple_open_job_one', 'jobcircle_jobs_simple_open_job_one');
add_action('wp_ajax_nopriv_jobcircle_jobs_simple_open_job_one', 'jobcircle_jobs_simple_open_job_one');
function jobcircle_jobs_simple_open_job_one()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_simple_open_job($atts);
