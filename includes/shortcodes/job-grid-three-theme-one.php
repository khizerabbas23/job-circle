<?php
function jobcircle_job_grid_listing()
{
	vc_map(
		array(
			'name' => __('Job Grid 40'),
			'base' => 'jobcircle_job_grid_listing',
...
add_action('vc_before_init', 'jobcircle_job_grid_listing');
...
function jobcircle_job_grid_listing_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
			'numofpost' => '',
			'page_view' => '',
		), $atts, 'jobcircle_job_grid_listing'
	);
			...
			<div class="container<?php echo esc_html($pageview); ?>">
				<div class="jobcircle-all-listing-con">
					...
					<header class="page-subheader mb-30 mb-md-40 d-lg-flex align-items-center justify-content-between">
						...
						$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
						if (!empty($job_count)) { ?>
							<h1 class="h4 mb-25 mb-lg-0">
								<?php esc_html_e('Showing 1 to', 'jobcircle-frame') ?>
								<?php echo esc_html($numofpost); ?>
								<?php esc_html_e('of', 'jobcircle-frame') ?>
								<?php echo esc_html($job_count); ?>
								<?php esc_html_e('jobs', 'jobcircle-frame') ?>
							</h1>
						<?php } ?>
							...
							<div class="grid-buttons">
								...
								if (!empty($listactive)) { ?>
									<a href="?view=list"><button class="btn <?php echo esc_html($listactive) ?>" type="button">
											<img src="<?php echo Jobcircle_Plugin::root_url() ?>/images/list-icon.svg" width="20"
												height="20" alt="List">
										</button></a>
								<?php }
								if (!empty($gridactive)) { ?>
									<a href="?view=grid"><button class="btn <?php echo esc_html($gridactive); ?>" type="button">
											<img src="<?php echo Jobcircle_Plugin::root_url() ?>/images/grid-icon.svg" width="22"
												height="22" alt="Grid">
										</button></a>
								<?php } ?>
					...
					<aside class="filters-sidebar custom-filters">
						<form method="post" class="jobcircle-jobfilter-form">
								...
								<div class="collapse show" id="collapseSalary">
									<div class="form-group">
										<div class="price-inputs">
											<input type="text" id="amount-start" name="min_salary" class="form-control"
												readonly placeholder="Form" value="">
											<input type="text" id="amount-end" name="max_salary" class="form-control"
												readonly placeholder="To" value="">
										</div>
										<div class="range-box">
							...
							<div class="filter-buttons">
								...
								<input type="hidden" name="action" value="jobcircle_jobs_grid_two_listing_ajax"><button
					...
					<div class="row jobcircle-alljobs-list">
						<?php echo jobcircle_jobs_grid_two_listing($atts); ?>
					</div>
				</div>
			</div>
		</section>
...
add_shortcode('jobcircle_job_grid_listing', 'jobcircle_job_grid_listing_frontend');
function jobcircle_jobs_grid_two_listing($atts)
{
	ob_start();
	$numofpost = isset($atts['numofpost']) ? $atts['numofpost'] : '';
	$orderby = isset($atts['orderby']) ? $atts['orderby'] : '';
	...
	if ($query->have_posts()) {
					...
					<div class="col-12 col-md-6 mb-15 mb-md-30">
						<article class="featured-box">
							<a href="" class="pin-job <?php echo esc_html($like_btn_class); ?> "
								data-id="<?php echo esc_html($postid); ?>"><i
							...
							<?php if (!empty($permalinkget) && !empty($job_img_url)) { ?>
								<div class="icon-box">
									<a href="<?php echo esc_html($permalinkget); ?>"><img src="<?php echo esc_url_raw($job_img_url) ?>"
											width="77" height="85" alt="Metronic Media Solution"></a>
								</div>
							<?php } ?>
							<div class="textbox">
				...
				<?php } else { ?>
					<div class="col-12 col-sm-6 col-lg-4 mb-15 mb-md-30">
						...
						<article class="featured-category-box alt">
							<a href="" class="fav-tag <?php echo esc_html($like_btn_class); ?> "
								data-id="<?php echo esc_html($postid) ?>"><i
									class=" <?php echo esc_html($fav_icon_class) ?> position-absolute <?php echo esc_html($follow) ?>"></i></a>
							<div>
								<?php if (!empty($job_img_url)) { ?>
									<div class="img-holder">
										<img src="<?php echo esc_url_raw($job_img_url) ?>" width="78" height="78"
											alt="Need senior rolling stock technician">
									</div>
								<?php } ?>
								<div class="textbox">
									<?php if (!empty($permalinkget) && !empty($title)) { ?>
									...
									if (!empty($post_author_url) && !empty($post_author_name)) { ?>
										<span class="subtitle">
											<?php esc_html_e('By', 'jobcircle-frame') ?><a href="<?php echo jobcircle_esc_the_html($post_author_url); ?>">
		...
		?>
			<p>
				<?php esc_html_e('No job found.', 'jobcircle-frame') ?>
			</p>
			<?php
	}
	add_action('wp_footer', function () {
			...
			<script>
				jQuery(document).on('click', '.jobcircle-follower-btn', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_fav_job_sixx_lis'
						},
						success: function (data) {
				...
				jQuery(document).on('click', '.jobcircle-alrdy-favjab', function () {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidate_remove_jobi_sixx_lis'
						},
						success: function (data) {
							var totalFavorites = data.total_favorites;
...
add_action('wp_ajax_jobcircle_jobs_grid_two_listing_ajax', 'jobcircle_jobs_grid_two_listing_ajax');
add_action('wp_ajax_nopriv_jobcircle_jobs_grid_two_listing_ajax', 'jobcircle_jobs_grid_two_listing_ajax');
function jobcircle_jobs_grid_two_listing_ajax()
{
	$atts = array(
		'numofpost' => $_POST['numposts'],
		'orderby' => $_POST['orderby']
	);
	$html = jobcircle_jobs_grid_two_listing($atts);
	wp_send_json(array('html' => $html));
}
// for favourite button
add_action('wp_ajax_jobcircle_candidate_fav_job_sixx_lis', 'jobcircle_candidate_fav_job_sixx_lis');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_sixx_lis', 'jobcircle_candidate_fav_job_sixx_lis');
function jobcircle_candidate_fav_job_sixx_lis()
{
	global $current_user;
	$user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_jobi_sixx_lis', 'jobcircle_candidate_remove_jobi_sixx_lis');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_jobi_sixx_lis', 'jobcircle_candidate_remove_jobi_sixx_lis');
function jobcircle_candidate_remove_jobi_sixx_lis()
{
	global $current_user;
	$user_id = $current_user->ID;
	$post_id = $_POST['post_id'];
