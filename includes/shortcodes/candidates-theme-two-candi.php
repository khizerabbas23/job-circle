<?php
function get_customs_posts_categories() {
...
function jobcircle_jobs_candidate_theme_2()
{
	vc_map(
		array(
			'name' => __('Candidates Theme 2'),
			'base' => 'jobcircle_jobs_candidate_theme_2',
			...
			'params' => array(
				...
				array(
					...
					'value' => array(
						...
						'6 Column Style' => 'column_style_six',
					),
				),
				  array(
					'type' => 'dropdown',
...
add_action('vc_before_init', 'jobcircle_jobs_candidate_theme_2');
...
function jobcircle_jobs_candidate_theme_2_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
		...
		$atts,
		'jobcircle_jobs_candidate_theme_2'
	);
	...
	<section class="section section-categories section-theme-1 pt-35 pt-md-50 pt-lg-75 pt-xl-95 pb-35 pb-md-50 pb-xl-75 theme_sevenb">
		<div class="container">
				<div class="jobcircle-all-listing-con">
					<!-- Page subheader -->
					<header class="page-subheader mb-40 mb-lg-70 d-lg-flex align-items-center justify-content-between">
						<?php
						$posts = get_posts([
					...
					<button class="btn btn-filters filters-opener opener-active" type="button">
						...
						<form method="post" class="jobcircle-jobfilter-form">
												...
												?>
														<li>
															<label class="custom-checkbox">
																<input id="cat-<?php echo jobcircle_esc_the_html($cat_term->term_id); ?>" name="job_categories" value="<?php echo jobcircle_esc_the_html($cat_term->slug); ?>" type="checkbox" <?php echo ($selectedcategory == $cat_term->slug)? 'checked': "" ; ?>>
							...
							<div class="filter-buttons">
								...
								<input type="hidden" name="action" value="jobcircle_candidates_themetwo_all"><button type="submit" class="btn btn-primary btn-sm"><span class="btn-text submit-filters-button"><?php esc_html_e('Apply Filter', 'jobcircle-frame') ?></span></button>
								<a href="#" class="btn btn-text btn-sm"><?php esc_html_e('Reset all filters', 'jobcircle-frame') ?></a>
							</div>
						</form>
					</aside>
					<div class="row jobcircle-alljobs-list">
						<?php echo jobcircle_candidates_theme_two($atts); ?>
					</div>
				</div>
			</div>
	</section>			
<?php
						...
						add_shortcode('jobcircle_jobs_candidate_theme_2', 'jobcircle_jobs_candidate_theme_2_frontend');

						function jobcircle_candidates_theme_two($atts)
						{
		...
		if (isset($_REQUEST['view']) && $_REQUEST['view'] == 'list') {
		    $numofpost -= 1;
							...
							// tax query
				// 			$tax_query = array();
							if (isset($_REQUEST['job_categories']) && $_REQUEST['job_categories'] != '' ) {
								$selectedcategory = $_REQUEST['job_categories'];
							}
							if($selectedcategory != ''){
								$tax_query = array(
								    array(
                            ...
                            if($atts['candidate_column_style'] == 'column_style_three') {
                                ...
                                $col_style = 'col-lg-4';
                            }elseif($atts['candidate_column_style'] == 'column_style_six') {
                                $col_style = 'col-lg-6';
                            }
							...
							if ($query->have_posts()) {
								...
								while ($query->have_posts()) {
									...
									$job_salary = jobcircle_candidate_salary_str($post);
                    				...
                    				$maxsalary = get_post_meta($post, 'jobcircle_field_max_salary', true);
					...
					<div class="col-12 col-md-6 <?php echo jobcircle_esc_the_html($col_style) ?> mb-20 mb-lg-30">
						<!-- Candidate Post -->
			...
			<?php
										...
										<div class="col-12 col-md-4 <?php echo jobcircle_esc_the_html($col_style) ?> mb-20 mb-lg-60">
									<!-- Candidate Post -->
         ...
         add_action('wp_footer', function () {
           ...
           data: {
               post_id: post_id,
               action: 'jobcircle_candidate_fav_job_liest_candiate'
           },
           success: function(data) {
           ...
           data: {
           post_id: post_id,
           action: 'jobcircle_candidate_remove_jobi_list_candiate'
           },
           success: function(data) {
           var totalFavorites = data.total_favorites;
	...
	?>
			<script>
				...
				jQuery(document).on('click', '.jobcircle-follower-btn', function() {
					...
					jQuery.ajax({
						...
						data: {
							post_id: post_id,
							action: 'jobcircle_candidates_themetwo_all'
						},
						success: function(data) {
							var totalFavorites = data.total_favorites;
							_this.removeClass('jobcircle-follower-btn');
		...
		<?php
					 ...
					 echo jobcircle_pagination($query, true);
						...
						add_action('wp_ajax_jobcircle_candidates_themetwo_all', 'jobcircle_candidates_themetwo_all');
						add_action('wp_ajax_nopriv_wp_ajax_jobcircle_candidates_themetwo_all', 'jobcircle_candidates_themetwo_all');

						function jobcircle_candidates_themetwo_all()
						{
							...
							$html = jobcircle_candidates_theme_two($atts);
...
add_action('wp_ajax_jobcircle_candidate_fav_job_liest_candiate', 'jobcircle_candidate_fav_job_liest_candiate');
add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_liest_candiate', 'jobcircle_candidate_fav_job_liest_candiate');
function jobcircle_candidate_fav_job_liest_candiate()
{
    global $current_user;
    $user_id = $current_user->ID;
...
add_action('wp_ajax_jobcircle_candidate_remove_jobi_list_candiate', 'jobcircle_candidate_remove_jobi_list_candiate');
add_action('wp_ajax_nopriv_jobcircle_candidate_remove_jobi_list_candiate', 'jobcircle_candidate_remove_jobi_list_candiate');
function jobcircle_candidate_remove_jobi_list_candiate()
{
    global $current_user;
    $user_id = $current_user->ID;
    $post_id = $_POST['post_id'];
