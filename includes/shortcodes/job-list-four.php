<?php
function job_circle_listed_four_jonbs()
{
	vc_map(
		array(
			'name' => __('Listed Job Four'),
			'base' => 'job_circle_listed_four_jonbs',
...
add_action('vc_before_init', 'job_circle_listed_four_jonbs');
...
function job_circle_listed_four_jonbs_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
						...
						<form class="form-search form-inline" action="#">
							<div class="fields-holder bg-white text-black d-flex flex-wrap flex-md-nowrap">
								<div class="form-group">
									...
									<input class="form-control" type="search" name="keyword" placeholder="<?php esc_html_e('Search Job Title', 'jobcircle-frame') ?>">
								</div>
								<div class="form-group">
									...
									<select class="select2" name="job_category" data-placeholder="<?php esc_html_e('Category', 'jobcircle-frame') ?>">
										...
										if (!empty($cat_terms)) {
											foreach ($cat_terms as $cat_term) {
										?>
												<option value="<?php echo esc_html($cat_term->slug) ?>"><?php echo esc_attr($cat_term->name); ?></option>
										<?php
											}
										}
										?>
				...
				<div class="row <?php echo esc_html($sidebaralign) ?>">
						...
						<aside class="filters-sidebar bg-light-gray">
							<form method="post" class="jobcircle-jobfilter-form">
								...
								<div class="filter-buttons">
									...
									<input type="hidden" name="action" value="jobcircle_simple_job_filters_submit_call_four"><button type="submit" class="btn btn-primary btn-sm"><span class="btn-text submit-filters-button"><?php esc_html_e('Filter', 'jobcircle-frame') ?></span></button>
					...
					<div class="col-12 col-lg-8">
						...
						<header class="page-subheader mb-30 mb-md-40 d-lg-flex align-items-center justify-content-between">
							<?php if (!empty($job_count)) { ?>
								<h1 class="h4 mb-25 mb-lg-0"><?php esc_html_e('Showing 1 to', 'jobcircle-frame'); ?>
									<?php echo esc_html($numofpost); ?>
									<?php esc_html_e('of', 'jobcircle-frame'); ?>
									<?php if (!empty($job_count)) {
						...
						<div class="row justify-content-center jobcircle-alljobs-list">
							<?php
													echo jobcircle_jobs_simple_list_four($atts); ?>
						</div>
					</div>
				</div>
			</div>
	...
	<?php
											...
											add_shortcode('job_circle_listed_four_jonbs', 'job_circle_listed_four_jonbs_frontend');
											function jobcircle_jobs_simple_list_four($atts)
											{
												ob_start();
	...
	<script>
		jQuery(document).on('click', '.jobcircle-follower-btn', function() {
			...
			jQuery.ajax({
				...
				data: {
					post_id: post_id,
					action: 'jobcircle_candidate_fav_job_list_four_list'
				},
				success: function(data) {
		...
		jQuery(document).on('click', '.jobcircle-alrdy-favjab', function() {
			...
			jQuery.ajax({
				...
				data: {
					post_id: post_id,
					action: 'jobcircle_candidate_remove_job_list_four_list'
				},
				success: function(data) {
					var totalFavorites = data.total_favorites;
...
<?php
											...
											add_action('wp_ajax_jobcircle_simple_job_filters_submit_call_four', 'jobcircle_simple_job_filters_submit_call_four');
											add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_submit_call_four', 'jobcircle_simple_job_filters_submit_call_four');
											function jobcircle_simple_job_filters_submit_call_four()
											{
												$atts = array(
													'numofpost' => $_POST['numposts'],
													'orderby' => $_POST['orderby']
												);
												$html = jobcircle_jobs_simple_list_four($atts);
												wp_send_json(array('html' => $html));
											}
											// for favourite button
											add_action('wp_ajax_jobcircle_candidate_fav_job_list_four_list', 'jobcircle_candidate_fav_job_list_four_list');
											add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_list_four_list', 'jobcircle_candidate_fav_job_list_four_list');
											function jobcircle_candidate_fav_job_list_four_list()
											{
												global $current_user;
												$user_id = $current_user->ID;
											...
											add_action('wp_ajax_jobcircle_candidate_remove_job_list_four_list', 'jobcircle_candidate_remove_job_list_four_list');
											add_action('wp_ajax_nopriv_jobcircle_candidate_remove_job_list_four_list', 'jobcircle_candidate_remove_job_list_four_list');
											function jobcircle_candidate_remove_job_list_four_list()
											{
												global $current_user;
												$user_id = $current_user->ID;
												$post_id = $_POST['post_id'];
