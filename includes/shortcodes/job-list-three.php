<?php
function jobcircle_job_list_three()
{
	vc_map(
		array(
			'name' => __('Job List Three'),
			'base' => 'jobcircle_job_list_three',
...
add_action('vc_before_init', 'jobcircle_job_list_three');
...
function jobcircle_job_list_three_frontend($atts, $content)
{
	$atts = shortcode_atts(
		array(
			'orderby' => '',
			...
			<div class="container">
				<div class="jobcircle-all-listing-con">
					...
					<button class="btn btn-filters filters-opener" type="button">
						...
						<form method="post" class="jobcircle-jobfilter-form">
							...
							<div class="filter-buttons">
								...
								<input type="hidden" name="action" value="jobcircle_jobs_listing_three_ajax"><button type="submit" class="btn btn-green btn-sm"><span class="btn-text"><?php esc_html_e('Apply Filter', 'jobcircle-frame') ?></span></button>
					...
					<div class="row jobcircle-alljobs-list">
						<?php
						echo jobcircle_jobs_listing_three($atts); ?>
					</div>
				</div>
			</div>
		</section>
	...
	add_shortcode('jobcircle_job_list_three', 'jobcircle_job_list_three_frontend');
	function jobcircle_jobs_listing_three($atts)
	{
		ob_start();
		$numofpost  = isset($atts['numofpost']) ? $atts['numofpost'] : '';
		$orderby  = isset($atts['orderby']) ? $atts['orderby'] : '';
		...
		if ($query->have_posts()) {
			...
			while ($query->have_posts()) {
				...
				$vacancies = get_post_meta($postid, 'jobcircle_field_vacancies', true);
				$job_type = get_post_meta($postid, 'jobcircle_field_job_type', true);
				$media = get_post_meta($postid, 'social_media', true);
							...
							<div class="textbox">
								...
								<strong class="subtitle">
									<?php if (!empty($permalinkget) && !empty($job_img_url)) { ?>
										<span class="icon">
											<a href="<?php echo esc_html($permalinkget); ?>"> <img src="<?php echo esc_url_raw($job_img_url); ?>" width="50" height="49" alt="Ever Media Studio">
											</a>
										</span>
										<?php }
									if (!empty($categories)) {
								...
								<?php } ?>
								<div class="jobcircle-aplybtn-con">
									<a href="javascript:;" class="jobcircle-det-applybtn jobdet-applybtn-act btn btn-primary " data-id="<?php echo esc_html($job_id) ?>" data-bs-toggle="modal" data-bs-target="#jobcircle-apply-job-popup"><span class="btn-text"><?php esc_html_e('Apply Job', 'jobcircle-frame'); ?></span></a>
								</div>
							</div>
						</article>
					</div>
				...
				<script>
					jQuery(document).on('click', '.jobcircle-follower-btn', function() {
						...
						jQuery.ajax({
							...
							data: {
								post_id: post_id,
								action: 'jobcircle_candidate_fav_job_list_three_list'
							},
							success: function(data) {
					...
					jQuery(document).on('click', '.jobcircle-alrdy-favjab', function() {
						...
						jQuery.ajax({
							...
							data: {
								post_id: post_id,
								action: 'jobcircle_candidate_remove_job_list_three_list'
							},
							success: function(data) {
								var totalFavorites = data.total_favorites;
	...
	add_action('wp_ajax_jobcircle_jobs_listing_three_ajax', 'jobcircle_jobs_listing_three_ajax');
	add_action('wp_ajax_nopriv_jobcircle_jobs_listing_three_ajax', 'jobcircle_jobs_listing_three_ajax');
	function jobcircle_jobs_listing_three_ajax()
	{
		$atts = array(
		...
		$html = jobcircle_jobs_listing_three($atts);

		wp_send_json(array('html' => $html));
	}
	// for favourite button
	add_action('wp_ajax_jobcircle_candidate_fav_job_list_three_list', 'jobcircle_candidate_fav_job_list_three_list');
	add_action('wp_ajax_nopriv_jobcircle_candidate_fav_job_list_three_list', 'jobcircle_candidate_fav_job_list_three_list');
	function jobcircle_candidate_fav_job_list_three_list()
	{
		global $current_user;
		$user_id = $current_user->ID;
	...
	add_action('wp_ajax_jobcircle_candidate_remove_job_list_three_list', 'jobcircle_candidate_remove_job_list_three_list');
	add_action('wp_ajax_nopriv_jobcircle_candidate_remove_job_list_three_list', 'jobcircle_candidate_remove_job_list_three_list');
	function jobcircle_candidate_remove_job_list_three_list()
	{
		global $current_user;
		$user_id = $current_user->ID;
		$post_id = $_POST['post_id'];
