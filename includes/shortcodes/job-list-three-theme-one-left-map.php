<?php
function jobcircle_new_job_lis_three_theme_one(){
    vc_map(
        array(
            'name' => __('job list 80'),
            'base' => 'jobcircle_new_job_lis_three_theme_one',
...
add_action('vc_before_init', 'jobcircle_new_job_lis_three_theme_one');
...
function jobcircle_new_job_lis_three_theme_one_frontend($atts, $content){
	$atts = shortcode_atts(
		array(
			'orderby' => '',
			'numofpost' => '',
		...
		$atts,
		'jobcircle_new_job_lis_three_theme_one'
	);
...
<main class="main">
			...
			<section class="section section-categories section-theme-1 pb-35 pb-lg-0 pt-0">
				<div class="container-fluid">
					...
					?>
						...
						<div class="filter-box">
							...
							<div class="filter-buttons">
										...
										<input type="hidden" name="action" value="jobcircle_simple_job_filters_new_submit_mapfiltornhi_call_one"><a href="#" class="btn btn-green btn-sm"><span class="btn-text"><?php esc_html_e('Apply Filter','jobcircle-frame')?></span></a>
					...
					<?php echo jobcircle_jobs_simple_new_list_oneasdfasdf($atts) ?>
					</div>
						</div>
...
add_shortcode('jobcircle_new_job_lis_three_theme_one', 'jobcircle_new_job_lis_three_theme_one_frontend');

function jobcircle_jobs_simple_new_list_oneasdfasdf($atts){
	ob_start();
    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
			...
			if ($post_employer_id > 0 && get_post_type($post_employer_id) == 'employer') {
				...
				$author_link = get_the_permalink($post_employer_id);
			} else {
...
$job_type_str = jobcircle_job_type_ret_str($job_type);
            ...
            <?php }else{  ?>
				<div class="col-12 col-xxxl-6 mb-15 mb-xl-30">
				<div class="jobs_info_holder">
					...
					<?php  } ?>		
						...
						<?php if(!empty($author_link) && !empty($post_author_name)){   ?>		
								<li>
									<a href="<?php echo esc_url_raw($author_link) ?>">
									<span class="text text-dark"><?php esc_html_e('By', 'jobcircle-frame') ?> <?php echo esc_html($post_author_name) ?></span>
									</a>
								</li>
						<?php } 
						if(!empty($job_location)){   ?>	
...
add_action('wp_ajax_jobcircle_simple_job_filters_new_submit_mapfiltornhi_call_one', 'jobcircle_simple_job_filters_new_submit_mapfiltornhi_call_one');
add_action('wp_ajax_nopriv_jobcircle_simple_job_filters_new_submit_mapfiltornhi_call_one', 'jobcircle_simple_job_filters_new_submit_mapfiltornhi_call_one');

function jobcircle_simple_job_filters_new_submit_mapfiltornhi_call_one()
{
	...
	$html = jobcircle_jobs_simple_new_list_oneasdfasdf($atts);
