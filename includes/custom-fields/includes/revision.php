...
if ( defined( 'JOBCIRCLE_CFS_REVISIONS' ) && JOBCIRCLE_CFS_REVISIONS ) {
...
class cstmfield_revision
{
    ...
    function _wp_post_revision_field_postmeta( $value, $column, $post ) {
        $output = '';
        $fields = JOBCIRCLE_CFS()->get( false, $post->ID );
        $field_info = JOBCIRCLE_CFS()->get_field_info( false, $post->ID );
    ...
    function check_for_changes( $default, $last_revision, $post ) {
        $revision_data = JOBCIRCLE_CFS()->get( false, $last_revision->ID );
        $post_data = JOBCIRCLE_CFS()->get( false, $post->ID );
    ...
    function save_post( $post_id ) {
        ...
        if ( $original_post_id ) {
            $field_data = JOBCIRCLE_CFS()->get( false, $original_post_id, [ 'format' => 'raw' ] );
            JOBCIRCLE_CFS()->save( $field_data, [ 'ID' => $post_id ] );
        }
    }
    ...
    function wp_restore_post_revision( $post_id, $revision_id ) {
        $field_data = JOBCIRCLE_CFS()->get( false, $revision_id, [ 'format' => 'raw' ] );
        JOBCIRCLE_CFS()->save( $field_data, [ 'ID' => $post_id ] );
    }
