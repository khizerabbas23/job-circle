    ...
    public function import( $options ) {
        ...
        if ( ! empty( $options['import_code'] ) ) {
            ...
            if ( ! empty( $stats['imported'] ) ) {
                $return .= '<div>' . __( 'Imported', 'jobcircle-frame' ) . ': ' . implode( ', ', $stats['imported'] ) . '</div>';
            }
            if ( ! empty( $stats['skipped'] ) ) {
                $return .= '<div>' . __( 'Skipped', 'jobcircle-frame' ) . ': ' . implode( ', ', $stats['skipped'] ) . '</div>';
            }
            return $return;
        }
        else {
            return '<div>' . __( 'Nothing to import', 'jobcircle-frame' ) . '</div>';
        }
    }
    ...
    */
    ...
    function save( $params = [] ) {
        ...
        foreach ( $params['fields'] as $key => $field ) {
            ...
            $field = JOBCIRCLE_CFS()->fields[ $field['type'] ]->pre_save_field( $field );
...
JOBCIRCLE_CFS()->field_group = new cstmfield_field_group();
