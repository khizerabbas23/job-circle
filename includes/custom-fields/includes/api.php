    ...
    */
    public function save_fields( $field_data = [], $post_data = [], $options = [] ) {
        ...
        if ( empty( $post_data['ID'] ) ) {
            $post_defaults = [
                'post_title'                => 'My JOBCIRCLE_CFS post',
                'post_content'              => '',
    ...
    */
    private function save_fields_recursive( $params ) {
        ...
        if ( isset( $field_array['value'] ) || 'loop' != $field_type ) {
            ...
            $values = JOBCIRCLE_CFS()->fields[ $field_type ]->pre_save( $values, $params['all_fields'][ $field_id ] );
    ...
    */
    public function get_input_fields( $params = [] ) {
        ...
        if (isset($params['custom_post_id'])) {
            $post_id = $params['custom_post_id'];
        }

        $values = $this->get_fields( $post_id, [ 'format' => 'input' ] );
    ...
    */
    private function assemble_value_array( &$field_data, $hierarchy, $field, $field_value = false, $options = false ) {
        ...
        if ( false !== $field_value ) {
            ...
            $value = JOBCIRCLE_CFS()->fields[ $field->type ]->prepare_value( $value, $field );

            if ( 'api' == $options['format'] ) {
                $value = JOBCIRCLE_CFS()->fields[ $field->type ]->format_value_for_api( $value, $field );
            }
            elseif ( 'input' == $options['format'] ) {
                $value = JOBCIRCLE_CFS()->fields[ $field->type ]->format_value_for_input( $value, $field );
            }
        }
    }
...
JOBCIRCLE_CFS()->api = new cstmfield_api();
