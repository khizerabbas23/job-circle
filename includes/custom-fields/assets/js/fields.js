(function($) {
    ...
    $(function() {
        ...
        $(document).on('click', '.cstmfield_add_field', function() {
            var html = JOBCIRCLE_CFS.field_clone.replace(/\[clone\]/g, '['+JOBCIRCLE_CFS.field_index+']');
            $('.fields').append('<li>' + html + '</li>');
            $('.fields li:last .field_label a').click();
            $('.fields li:last .field_type select').change();
            JOBCIRCLE_CFS.field_index = JOBCIRCLE_CFS.field_index + 1;
            init_tooltip();
        });
        ...
        $(document).on('change', '.field_form .field_type select', function() {
            ...
            var html = JOBCIRCLE_CFS.options_html[type].replace(/cstmfield\[fields\]\[clone\]/g, input_name);
