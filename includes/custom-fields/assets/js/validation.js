(function($) {
    $(function() {
        ...
        JOBCIRCLE_CFS.is_draft = false;
        $(document).on('click', '#save-post', function() {
            JOBCIRCLE_CFS.is_draft = true;
        });

        $('form#post').submit(function() {

            // skip validation for drafts
            if (false === JOBCIRCLE_CFS.is_draft) {
                var passthru = true;

                // handle each validator field
                $.each(JOBCIRCLE_CFS.field_rules, function(field_name, obj) {
                    $('.cstmfield_input .field-' + field_name).each(function() {
                        ...
                        if ('object' == typeof JOBCIRCLE_CFS.validators[validator]) {

                            // set the DOM attribute
                            $this.attr('data-validator', obj.rule);

                            // figure out the field value
                            if ('function' == typeof JOBCIRCLE_CFS.get_field_value[type]) {
                                var val = JOBCIRCLE_CFS.get_field_value[type]($this);
                            }
                            else {
                            ...
                            var is_valid = JOBCIRCLE_CFS.validators[validator]['validate'](val, $this);
                            ...
                            if (!is_valid) {
                                ...
                                var error_msg = JOBCIRCLE_CFS.validators[validator]['error'];
