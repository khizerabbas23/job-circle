 ...
 */
if (!class_exists('jobcircle_post_categories')) {
    class jobcircle_post_categories extends WP_Widget {
        /**
         * Sets up a new base-frame widget instance.
         */
        public function __construct() {
            parent::__construct(
                'jobcircle_post_categories',
                // Base ID.
                __('Blog Weightes Categories', 'base-frame'),
                // Name.
                array('classname' => 'widget_footer_contact', 'description' => __('Bloag Page Category', 'base-frame'))
            );
        }
                        ...
                        foreach ($categories as $category) {
                            ...
                            if ($counter >= 0) {
                               ...
                               <?php if(!empty($term_link || $category || $category) ){
                                    ?>
								 <a href="<?php echo esc_url($term_link); ?>"><?php echo esc_html($category->name); ?></a>  <span class="count"><?php echo $category->count ?></span></li>
                                <?php
                                }
                                ?>
...
add_action('widgets_init', function () {
    register_widget("jobcircle_post_categories");
});
