...
// Don't duplicate me!
if (!class_exists('ReduxFramework_jobcircle_multi_select')) {
     ...
     * @since       1.0.0
     */
    class ReduxFramework_jobcircle_multi_select extends ReduxFramework {
        ...
        public function render() {
            ...
            $field_value = isset($jobcircle_framework_options[$field_id]) ? $jobcircle_framework_options[$field_id] : '';
            ...
            $output = '
			<div id="multi-select-text-' . $field_random . '" class="jobcircle-multi-select-text">
				<ul>
            ...
            if (isset($field_value[$field_select_name]) && is_array($field_value[$field_select_name])) {
                ...
                foreach ($field_value[$field_select_name] as $field_select_val) {
                    ...
                    if ($field_counter > 0) {
                        $output .= '<div class="remove-field"><a href="javascript:void(0)">' . __('Remove', 'jobcircle-frame') . '</a></div>';
                    }
                    $output .= '
							</li>';
                    $field_counter ++;
            ...
            $output .= '
				...
				<a href="javascript:void(0)" class="add-more add-mor-' . $field_random . '" data-id="' . $field_random . '">' . $field_add_more . '</a>
				<script>
				jQuery(document).ready(function($){
					$(document).on(\'click\', \'.jobcircle-multi-select-text > .add-mor-' . $field_random . '\', function() {
						var field_id_' . $field_random . ' = $(this).data(\'id\');
						var app_html_' . $field_random . ' = \'\
						<li>\
							<div class="select-field">\
								<select name="' . $field_name . '[' . $field_select_name . '][]">\
									' . $select_options_html . '\
							...
							<div class="remove-field"><a href="javascript:void(0)">' . __('Remove', 'jobcircle-frame') . '</a></div>\
						</li>\';
						$(\'#multi-select-text-\' + field_id_' . $field_random . ' + \' ul\').append(app_html_' . $field_random . ');
					});
					$(document).on(\'click\', \'.jobcircle-multi-select-text .remove-field\', function() {
        ...
        public function enqueue() {
            ...
            $extension = ReduxFramework_extension_jobcircle_multi_select::getInstance();

            wp_enqueue_script(
                    'jobcircle-redux-multi-select-js', $this->extension_url . 'field_multi_select.js', array('jquery'), time(), true
            );

            wp_enqueue_style(
                    'jobcircle-redux-multi-select-css', $this->extension_url . 'field_multi_select.css', time(), true
            );
        }
