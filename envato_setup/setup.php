...
if (!class_exists('Envato_Theme_Setup_Wizard')) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        /**
         * jobcircle_tgmpa instance storage
         *
         * @var object
         */
        protected $jobcircle_tgmpa_instance;

        /**
         * jobcircle_tgmpa Menu slug
         *
         * @var string
         */
        protected $jobcircle_tgmpa_menu_slug = 'jobcircle-install-plugins';

        /**
         * jobcircle_tgmpa Menu url
         *
         * @var string
         */
        protected $jobcircle_tgmpa_url = 'themes.php?page=jobcircle-install-plugins';
        ...
        public function init_globals() {
            ...
            $this->plugin_url = jobcircle_framework_get_url('envato_setup');
        }
        ...
        public function init_actions() {
            if (apply_filters($this->theme_name . '_enable_setup_wizard', true) && current_user_can('manage_options')) {
                ...
                if (class_exists('TGM_Plugin_Activation') && isset($GLOBALS['jobcircle_tgmpa'])) {
                    add_action('init', array($this, 'get_jobcircle_tgmpa_instanse'), 30);
                    add_action('init', array($this, 'set_jobcircle_tgmpa_url'), 40);
                }
                ...
                add_filter('jobcircle_tgmpa_load', array($this, 'jobcircle_tgmpa_load'), 10, 1);
        ...
        public function jobcircle_tgmpa_load($status) {
        ...
        /**
         * Get configured jobcircle_tgmpa instance
         *
         * @access public
         * @since 1.1.2
         */
        public function get_jobcircle_tgmpa_instanse() {
            $this->jobcircle_tgmpa_instance = call_user_func(array(get_class($GLOBALS['jobcircle_tgmpa']), 'get_instance'));
        }

        /**
         * Update $jobcircle_tgmpa_menu_slug and $jobcircle_tgmpa_parent_slug from jobcircle_tgmpa instance
         *
         * @access public
         * @since 1.1.2
         */
        public function set_jobcircle_tgmpa_url() {

            $this->jobcircle_tgmpa_menu_slug = ( property_exists($this->jobcircle_tgmpa_instance, 'menu') ) ? $this->jobcircle_tgmpa_instance->menu : $this->jobcircle_tgmpa_menu_slug;
            $this->jobcircle_tgmpa_menu_slug = apply_filters($this->theme_name . '_theme_setup_wizard_jobcircle_tgmpa_menu_slug', $this->jobcircle_tgmpa_menu_slug);

            $jobcircle_tgmpa_parent_slug = ( property_exists($this->jobcircle_tgmpa_instance, 'parent_slug') && $this->jobcircle_tgmpa_instance->parent_slug !== 'themes.php' ) ? 'admin.php' : 'themes.php';

            $this->jobcircle_tgmpa_url = apply_filters($this->theme_name . '_theme_setup_wizard_jobcircle_tgmpa_url', $jobcircle_tgmpa_parent_slug . '?page=' . $this->jobcircle_tgmpa_menu_slug);
        }
        ...
        public function init_wizard_steps() {
            ...
            if (class_exists('Jobcircle_TGM_Plugin_Activation') && isset($GLOBALS['jobcircle_tgmpa'])) {
        ...
        public function setup_wizard() {
            ...
            wp_localize_script('envato-setup', 'envato_setup_params', array(
                'tgm_plugin_nonce' => array(
                    'update' => wp_create_nonce('jobcircle_tgmpa-update'),
                    'install' => wp_create_nonce('jobcircle_tgmpa-install'),
                ),
                'tgm_bulk_url' => admin_url($this->jobcircle_tgmpa_url),
        ...
        public function envato_setup_introduction() {
            ...
            if (false && isset($_REQUEST['debug'])) {
                ...
                ?>
                <h1><?php printf(esc_html__('Welcome to the setup wizard for %s.', 'jobcircle-frame'), wp_get_theme()); ?></h1>
                <p><?php esc_html_e('It looks like you have already run the setup wizard. Below are some options: ', 'jobcircle-frame'); ?></p>
                <ul>
                    <li>
                        <a href="<?php echo esc_url($this->get_next_step_link()); ?>"
                           class="button-primary button button-next button-large"><?php esc_html_e('Run Import Demo Data Again', 'jobcircle-frame'); ?></a>
                    </li>
                    <li>
                        <form method="post">
                            <input type="hidden" name="reset-font-defaults" value="yes">
                                <input type="submit" class="button-primary button button-large button-next"
                                       value="<?php esc_attr_e('Reset font style and colors', 'jobcircle-frame'); ?>" name="save_step"/>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url(wp_get_referer() && !strpos(wp_get_referer(), 'update.php') ? wp_get_referer() : admin_url('') ); ?>"
                       class="button button-large"><?php esc_html_e('Cancel', 'jobcircle-frame'); ?></a>
                </p>
                <?php
            } else {
                ?>
                <h1><?php printf(esc_html__('Welcome to the setup wizard for %s.', 'jobcircle-frame'), wp_get_theme()); ?></h1>
                <p><?php printf(esc_html__('Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'jobcircle-frame'), wp_get_theme()); ?></p>
                <p><?php esc_html_e('No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'jobcircle-frame'); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url($this->get_next_step_link()); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e('Let\'s Go!'); ?></a>
                    <a href="<?php echo esc_url(wp_get_referer() && !strpos(wp_get_referer(), 'update.php') ? wp_get_referer() : admin_url('') ); ?>"
                       class="button button-large"><?php esc_html_e('Not right now', 'jobcircle-frame'); ?></a>
                </p>
                <?php
            }
        }
        ...
        public function envato_setup_introduction_save() {
            ...
            if (!empty($_POST['reset-font-defaults']) && $_POST['reset-font-defaults'] == 'yes') {
                ...
                ?>
                <p>
                    <strong><?php esc_html_e('Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'jobcircle-frame'); ?></strong>
                </p>
                <?php
                return true;
            }
        ...
        private function _get_plugins() {
            $instance = call_user_func(array(get_class($GLOBALS['jobcircle_tgmpa']), 'get_instance'));
            $plugins = array(
        ...
        public function envato_setup_default_plugins() {
            ...
            jobcircle_tgmpa_load_bulk_installer();
            ...
            <form method="post">
                ...
                if (count($plugins['all'])) {
                    ?>
                    <p><?php esc_html_e('Your website needs a few essential plugins. The following plugins will be installed or updated:', 'jobcircle-frame'); ?></p>
                    ...
                    echo '<p><strong>' . esc_html_e('Good news! All plugins are already installed and up to date. Please continue.', 'jobcircle-frame') . '</strong></p>';
                }
                ?>

                <p><?php esc_html_e('You can add and remove plugins later on from within WordPress.', 'jobcircle-frame'); ?></p>

                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url($this->get_next_step_link()); ?>"
                       class="button-primary button button-large button-next"
                       data-callback="install_plugins"><?php esc_html_e('Continue', 'jobcircle-frame'); ?></a>
                    <a href="<?php echo esc_url($this->get_next_step_link()); ?>"
                       class="button button-large button-next"><?php esc_html_e('Skip this step', 'jobcircle-frame'); ?></a>
                       <?php wp_nonce_field('envato-setup'); ?>
                </p>
            </form>
            <?php
        ...
        public function ajax_plugins() {
            if (!check_ajax_referer('envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])) {
                wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'jobcircle-frame')));
            }
            $json = array();
            ...
            foreach ($plugins['activate'] as $slug => $plugin) {
                if ($_POST['slug'] == $slug) {
                    $json = array(
                        'url' => admin_url($this->jobcircle_tgmpa_url),
                        'plugin' => array($slug),
                        'jobcircle_tgmpa-page' => $this->jobcircle_tgmpa_menu_slug,
                        'plugin_status' => 'all',
                        '_wpnonce' => wp_create_nonce('bulk-plugins'),
                        'action' => 'jobcircle_tgmpa-bulk-activate',
                        'action2' => - 1,
            ...
            foreach ($plugins['update'] as $slug => $plugin) {
                if ($_POST['slug'] == $slug) {
                    $json = array(
                        ...
                        'plugin_status' => 'all',
                        '_wpnonce' => wp_create_nonce('bulk-plugins'),
                        'action' => 'jobcircle_tgmpa-bulk-update',
                        'action2' => - 1,
                        'message' => esc_html__('Updating Plugin', 'jobcircle-frame'),
                    );
                    break;
                }
            }
            foreach ($plugins['install'] as $slug => $plugin) {
                if ($_POST['slug'] == $slug) {
                    $json = array(
                        ...
                        'plugin_status' => 'all',
                        '_wpnonce' => wp_create_nonce('bulk-plugins'),
                        'action' => 'jobcircle_tgmpa-bulk-install',
                        'action2' => - 1,
                        'message' => esc_html__('Installing Plugin', 'jobcircle-frame'),
                    );
                    break;
                }
            }
        ...
        private function _content_rev_slider_install() {
            ...
            if (function_exists('jobcircle_demo_data_get_path')) {
                $theme_style = jobcircle_demo_data_get_path('content/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '/');
        ...
        private function _content_users_install() {
            ...
            if (!empty($jobs_list)) {
                ...
                foreach ($jobs_list as $job_item) {
                    ...
                    update_post_meta($job_id, 'jobcircle_field_job_posted_by', $employer_id);
                    update_post_meta($job_id, 'jobcircle_job_employer_status', 'approved');
                    $job_count++;
                }
            }
            //
            ...
            if (!empty($candidates_list)) {
                ...
                foreach ($candidates_list as $candidate_item) {
                    ...
                    $user_arr['role'] = 'jobcircle_candidate';
            ...
            if (!empty($employers_list)) {
                ...
                foreach ($employers_list as $employer_item) {
                    ...
                    $user_arr['nickname'] = $first_name;
                    $user_arr['display_name'] = $post_title;
                    $user_arr['role'] = 'jobcircle_employer';
                ...
                foreach ($users_array as $user) {
                    ...
                    if (!email_exists($email)) {
                        ...
                        if (isset($user['role']) && $user['role'] != '') {
                            if ($user['role'] == 'jobcircle_employer') {
                                wp_update_user(array('ID' => $id, 'role' => 'jobcircle_employer'));

                                if (isset($user['member_id']) && $user['member_id'] != '') {
                                    update_user_meta($id, 'jobcircle_employer_id', $user['member_id']);
                                    update_post_meta($user['member_id'], 'jobcircle_user_id', $id);
                                }
                            } else {
                                wp_update_user(array('ID' => $id, 'role' => 'jobcircle_candidate'));

                                if (isset($user['member_id']) && $user['member_id'] != '') {
                                    update_user_meta($id, 'jobcircle_candidate_id', $user['member_id']);
                                    update_post_meta($user['member_id'], 'jobcircle_user_id', $id);
                                }
                            }
                        }
        ...
        private function _fetch_remote_file($url, $post) {
            ...
            if (function_exists('jobcircle_demo_data_get_path')) {
                $local_file = jobcircle_demo_data_get_path('content/images/') . $file_name;
            } else {
                $local_file = jobcircle_framework_get_path('demo-data-setup/content/images/stock/') . $file_name;
            }
            $upload = false;
        ...
        public function _content_wpbakery_templates() {
            ...
            if (function_exists('jobcircle_demo_data_get_path')) {
                ...
                $wp_bakery_dir = jobcircle_demo_data_get_path('content/') . 'wp_bakery_templates/';
        ...
        public function _theme_options_install() {
            ...
            if (isset($custom_options['jobcircle_framework_options'])) {
                $optionsss = json_decode($custom_options['jobcircle_framework_options'], true);
                //$redux = ReduxFrameworkInstances::get_instance('jobcircle_framework_options');
                //$redux->set_options($optionsss);
                update_option('jobcircle_framework_options', $optionsss);
            }
            ...
            if (isset($optionsss['jobcircle-site-logo']['url'])) {
                $get_thoptions = get_option('jobcircle_framework_options');
                $get_thoptions['jobcircle-site-logo']['url'] = $optionsss['jobcircle-site-logo']['url'];
                update_option('jobcircle_framework_options', $get_thoptions);
            }
        ...
        public function _get_json($file, $path = '') {
            ...
            if (is_file($theme_style . basename($file))) {
                ...
                if (file_exists($file_name)) {
                    ...
                    if (strpos($file_content, 'https:\\\/\\\/jobcircle.net\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/jobcircle.net\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/';
                        $file_content = str_replace(array($find_url), array($replace_url_with), $file_content);
                    } else if (strpos($file_content, 'https:\\\/\\\/jobcircle.net\\\/demo\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/jobcircle.net\\\/demo\\\/';
        ...
        public function envato_setup_logo_design_save() {
            ...
            global $JobcircleFrameReduxFramework;
            $theme_options = get_option('jobcircle_framework_options');
            ...
            if ($new_logo_id) {
                ...
                if ($attr && !empty($attr[1]) && !empty($attr[2])) {
                    ...
                    if ($attr && isset($attr[0]) && !empty($attr[0])) {
                        ...
                        //$JobcircleFrameReduxFramework->ReduxFramework->set('jobcircle-site-logo', $logo_image);
                        $theme_options['jobcircle-site-logo']['url'] = $attr[0];
                        update_option('jobcircle_framework_options', $theme_options);
                    }
                }
            } else {
                $image_url = $this->plugin_url . '/images/' . get_theme_mod('dtbwp_site_style', $this->get_default_theme_style()) . '/logo.png';
                ...
                //$JobcircleFrameReduxFramework->ReduxFramework->set('jobcircle-site-logo', $logo_image);
                $theme_options['jobcircle-site-logo']['url'] = $image_url;
                update_option('jobcircle_framework_options', $theme_options);
            }
